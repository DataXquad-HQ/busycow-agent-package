...
description: >
  Generate invoices for DataXquad clients after a quotation is accepted or a
...
## Base & Tables
- **Base Token:** `MtvNbgCHXaRAaUsWXsCjnekep2g`
...
| Notes | `{{FIELD_ID}}` | Text |
| Invoices (link) | `{{FIELD_ID}}` | DuplexLink → Invoices table |

Items are linked to the Invoice via `{{FIELD_ID}}` (DuplexLink). Always pull items by filtering on this field using the Invoice record ID.
...
| `{ENTITY}` | `ATA` = ATA Limited 應科聯有限公司 (TW entity) · `DX` = DataXquad Pte. Ltd. (SG entity) |
...
| GeoKernel / TRACI product | ATA Limited 應科聯 | `ATA` |
| Overseas client (HK, SG, MY) | DataXquad Pte. Ltd. | `DX` |
| Hardware/IoT product sales (any market) | ATA Limited 應科聯 | `ATA` |
| BusyCow / AquaOptima product | DataXquad Pte. Ltd. | `DX` |
...
- **Google Drive upload**: Invoices folder is in the DataXquad shared drive — always add `?supportsAllDrives=true` to the upload URL or you get a 404. See `references/gdocs-workflow.md` Pitfalls section.
- **Lark Base Entity field**: This is a SingleSelect. Existing options: "DataXquad Pte. Ltd." (SG) and "ATA Limited 應科聯有限公司" (TW). Pass the full string exactly — Lark will create a new option if it doesn't match, causing duplicate entries.
- **Accounts table primary field is "Company Name"** (NOT "Client Name", NOT "Account Name") — filter by `Company Name` when searching for a client record. `Client Name` returns `InvalidFilter` error.
- **DuplexLink fields require a plain array of strings**: `["record_id"]` — NOT `{"link_record_ids": ["record_id"]}`. Passing the object form returns error 1254074 "parameters of Duplex Link field are invalid".
```
