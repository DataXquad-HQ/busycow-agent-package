...
When a cron-based workflow breaks, pauses, or needs a replay, separate **schedule recovery** from **report logic recovery**.

Read as needed:
- `references/cron-recovery.md` — when to use `list`, `resume`, `run`, `update`, or `remove`

Core rule:
- `resume` restores a paused recurring schedule
- `run` performs an immediate manual retry / verification run
- if the job logic is wrong, `update` first, then `run`
- always `list` first; never guess the `job_id`

A repaired cron workflow is not complete until both are true:
1. the recurring schedule is in the right state
2. the report delivery path is working again
