...
When a skill should be available across multiple agent profiles, use the
**canonical-source + per-profile copies** pattern.

### Canonical source
Maintain the source skill here:
...
### Runtime copies
Distribute real copies into the profiles that actually need it:
...
### Package copies
When publishing externally, package shared skills under:

```text
artifacts/shared-skills/<skill-name>/
```

and agent-local runtime copies under:
...
- **Agent profile skills are not backed up by nightly brain sync** —
  ...
  profile deletion.
- **Do not use `shared_skills/` registries or `_shared/` symlink trees as the default pattern.**
  The live governance model is canonical source + per-profile copies.
